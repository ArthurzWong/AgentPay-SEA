...
function App() {
  ...
  return <><div className="space" aria-hidden="true"><div className="holo-grid"/><div className="holo-core"><span/><span/><span/></div><span className="star s1"/><span className="star s2"/><span className="star s3"/><span className="star s4"/><span className="star s5"/><span className="star s6"/><span className="star s7"/><span className="star s8"/></div><main>
    ...
    <section className="console"><div className="section-title"><div><span className="eyebrow">AGENT CONSOLE</span><h3>What should the agent do?</h3></div><span className="guardrail">AUTO-APPROVE ≤ $0.010</span></div><label className="sr-only" htmlFor="agent-task">Agent task</label><textarea id="agent-task" value={prompt} maxLength={500} onChange={e => setPrompt(e.target.value)} aria-label="Agent task"/>
    ...
    <div className="grid">
      <section className="panel activity"><div className="section-title"><div><span className="eyebrow">LIVE TRACE</span><h3>Agent Activity</h3></div>{loading && <span className="running">● LIVE</span>}</div>
        ...
        {run?.activities.map((a, i) => <div className={`event ${a.tone}`} key={i}><span className="dot"/><div><b>{a.label}</b>{a.detail && <p>{a.signature ? <a href={`https://explorer.solana.com/tx/${a.signature}?cluster=devnet`} target="_blank" rel="noreferrer">{short(a.detail)}</a> : a.detail}</p>}</div></div>)}
      </section>
